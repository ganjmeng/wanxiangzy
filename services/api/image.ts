...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    return requestPlatformImage(config, prompt, [], options);
}

export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    const requestPrompt = buildImageReferencePromptText(prompt, references);
    const requestReferences = mask ? [...references, mask] : references;
    return requestPlatformImage(config, requestPrompt, requestReferences, options);
}

export async function requestImageQuestion(_config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    const response = await fetch("/api/infinite-canvas/text", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages }),
        signal: options?.signal,
    });
    if (!response.ok) throw new Error(await readPlatformFetchError(response, "文本生成失败"));
    const payload = (await response.json()) as { text?: string; error?: string };
    const answer = payload.text || payload.error || "没有返回内容";
    onDelta(answer);
    return answer;
}

type PlatformGenerationCreateResponse = {
    generation_id?: string;
    error?: string;
};

type PlatformGenerationStatusResponse = {
    status?: string;
    status_group?: string;
    result_urls?: string[];
    error?: string;
...
export type PlatformImageGenerationTask = {
    provider: "platform";
    id: string;
    path: string;
    model: string;
...
export type PlatformImageGenerationTaskState = {
    status: "processing" | "completed" | "failed";
    resultUrls: string[];
    error?: string;
};

export async function createPlatformImageGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<PlatformImageGenerationTask> {
    const mode = references.length ? "image-to-image" : "text-to-image";
    const model = normalizePlatformImageModel(config.model || config.imageModel);
    ...
    const response = await fetch("/api/general-image", {
        ...
        body: JSON.stringify({
            ...
            reference_urls: await Promise.all(references.map(platformImageReferenceUrl)),
            ai_model: model,
            aspect_ratio: normalizePlatformAspectRatio(config.size),
            image_size: normalizePlatformImageSize(config.quality),
            gen_count: normalizePlatformCount(config.count),
            module_kind: "generalImage",
        }),
        signal: options?.signal,
    });
    if (!response.ok) throw new Error(await readPlatformFetchError(response, "Image generation task creation failed"));

    const created = (await response.json()) as PlatformGenerationCreateResponse;
    if (!created.generation_id) throw new Error(created.error || "Image generation task id missing");

    return { provider: "platform", id: created.generation_id, path: "/api/general-image", model, mode };
}

export async function pollPlatformImageGenerationTask(task: PlatformImageGenerationTask, options?: RequestOptions): Promise<PlatformImageGenerationTaskState> {
    const response = await fetch(`${task.path || "/api/general-image"}?generation_id=${encodeURIComponent(task.id)}`, { signal: options?.signal });
    if (!response.ok) throw new Error(await readPlatformFetchError(response, "Image generation task status failed"));
    const payload = (await response.json()) as PlatformGenerationStatusResponse;
    return {
        status: normalizeStatus(payload.status_group || payload.status),
        resultUrls: (payload.result_urls || []).filter(Boolean),
        error: payload.error,
    };
}

export async function waitForPlatformImageGenerationTask(task: PlatformImageGenerationTask, options?: RequestOptions) {
    const urls = await pollPlatformGeneration(task.path || "/api/general-image", task.id, options?.signal);
    if (!urls.length) throw new Error("Image generation returned no result");
    return urls;
}

async function requestPlatformImage(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    const response = await fetch("/api/general-image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            ...
            reference_urls: await Promise.all(references.map(platformImageReferenceUrl)),
            ai_model: normalizePlatformImageModel(config.model || config.imageModel),
            aspect_ratio: normalizePlatformAspectRatio(config.size),
            image_size: normalizePlatformImageSize(config.quality),
            gen_count: normalizePlatformCount(config.count),
            module_kind: "generalImage",
    ...
    if (!response.ok) throw new Error(await readPlatformFetchError(response, "图片生成任务创建失败"));

    const created = (await response.json()) as PlatformGenerationCreateResponse;
    if (!created.generation_id) throw new Error(created.error || "图片生成接口没有返回任务 ID");

    const urls = await pollPlatformGeneration("/api/general-image", created.generation_id, options?.signal);
    if (!urls.length) throw new Error("图片生成接口没有返回结果");
    return urls.map((dataUrl) => ({ id: nanoid(), dataUrl }));
}

async function pollPlatformGeneration(path: string, generationId: string, signal?: AbortSignal) {
    for (let attempt = 0; attempt < 180; attempt += 1) {
        if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
        await delay(attempt < 2 ? 800 : 2000, signal);
        const response = await fetch(`${path}?generation_id=${encodeURIComponent(generationId)}`, { signal });
        if (!response.ok) throw new Error(await readPlatformFetchError(response, "生成任务查询失败"));
        const payload = (await response.json()) as PlatformGenerationStatusResponse;
        const status = normalizeStatus(payload.status_group || payload.status);
        if (status === "completed") return (payload.result_urls || []).filter(Boolean);
        if (status === "failed") throw new Error(payload.error || "生成失败");
    }
    throw new Error("生成超时，请稍后在作品历史中查看结果");
}

function normalizePlatformImageModel(value: string) {
    const model = modelOptionName(value);
    if (model === "nano-banana-pro" || model === "nano-banana-2" || model === "gpt-image-2") return model;
    return "nano-banana-2";
}

function normalizePlatformImageSize(value: string) {
...
function normalizePlatformAspectRatio(value: string) {
    const normalized = value.trim();
    const supported = new Set(["auto", "1:1", "9:16", "16:9", "4:3", "3:4", "2:3", "3:2", "4:5", "5:4", "21:9"]);
    if (supported.has(normalized)) return normalized;
    if (/^\d+x\d+$/i.test(normalized)) return "auto";
    return "auto";
}

function normalizePlatformCount(value: string) {
    const count = Math.floor(Math.abs(Number(value)) || 1);
    return Math.max(1, Math.min(4, count));
}

async function platformImageReferenceUrl(image: ReferenceImage) {
...
async function readPlatformFetchError(response: Response, fallback: string) {
    ...
    if (response.status === 401 || response.status === 403) return "请先登录后使用画布生成";
