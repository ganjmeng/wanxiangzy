...
// Hard-rule marker for idempotency + leading segment (≤280 chars).
const MATERIAL_HARD_RULE_MARK = "【HARD 硬规则 · 材质增强模式】";
const MATERIAL_HARD_RULE = `${MATERIAL_HARD_RULE_MARK}
1) 允许改变（仅限图1可见服装区域）：面料纹理清晰度、织纹层次、缝线/压线、纽扣/拉链/五金、刺绣、logo 边缘和已有真实褶皱的可见度。
2) 必须不变：人物身份、五官、发型、肤色、身体比例、姿势、服装款式、版型、轮廓、长度、穿着位置、固有颜色、图案位置、logo 形状和位置、背景、构图、镜头距离、光线方向、曝光、阴影、景深。
3) 冲突时图1原图优先；图2只能补足材质细节，必须贴合图1当前可见区域，禁止脑补不可见细节。`;
...
export function buildMaterialEnhancementPrompt(params: {
  ...
  // 7 段结构（v2）：硬规则 → 图像角色 → 任务 → 服装保真 + 编辑边界 → 增强强度 → 材质表现 + 细密纹理 → 用户/质量/负面
  return [
    MATERIAL_HARD_RULE,
...
export function enforceMaterialEnhancementPromptRequirements(prompt: string, params: {
  ...
  if (normalized.includes(MATERIAL_HARD_RULE_MARK)) return normalized;
