...
export function fitNodeSize(width: number, height: number, maxWidth = 640, maxHeight = 640) {
    // Unknown dimensions (e.g. CORS-blocked remote images where we couldn't probe size):
    // fall back to the max frame so the node is still visible at a reasonable size.
