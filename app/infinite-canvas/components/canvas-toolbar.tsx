...
import { Button, Segmented, Switch } from "@/components/ui/shadcn-compat";
...
export function CanvasToolbar({
    ...
    const hoverStyle = { background: colorTheme === "dark" ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.05)" };
    ...
    return (
        <div className="pointer-events-none fixed bottom-3 left-1/2 z-50 -translate-x-1/2">
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-10 max-w-[calc(100vw-32px)] items-center gap-0.5 overflow-x-auto rounded-lg border px-1 shadow-sm [&>*]:shrink-0" style={dockStyle}>
            ...
            {appearanceOpen ? (
                <div
                    className="pointer-events-auto absolute bottom-[52px] right-0 z-30 w-[248px] rounded-lg border p-2.5 shadow-lg"
                    style={{ background: colorTheme === "dark" ? "rgba(20,20,22,.96)" : "rgba(255,255,255,.98)", borderColor: colorTheme === "dark" ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.08)", color: theme.node.text }}
                >
                    ...
                    <div className="grid grid-cols-2 gap-1 rounded-md p-0.5" style={{ background: colorTheme === "dark" ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)" }}>
                    ...
                    <Segmented
                        className="w-full !p-1 [&_.ant-segmented-group]:!flex [&_.ant-segmented-item]:!min-h-7 [&_.ant-segmented-item]:!flex-1 [&_.ant-segmented-item-label]:!min-h-7 [&_.ant-segmented-item-label]:!leading-7"
                    ...
                    />
                    <div className="mt-3 flex items-center justify-between gap-3 rounded-md px-1.5 py-1">
...
function ToolbarButton({
    ...
    return (
        <Button
            ...
            disabled={disabled}
            style={active ? activeStyle : hovered === id && !disabled ? hoverStyle : { color: danger ? "#f87171" : theme.node.text, opacity: disabled ? 0.35 : 1 }}
            icon={children}
            onMouseEnter={(event) => {
                onHover(id);
...
function Divider({ theme }: { theme: CanvasTheme }) {
    return <div className="mx-0.5 h-4 w-px" style={{ background: theme.toolbar.border, opacity: 0.6 }} />;
}
