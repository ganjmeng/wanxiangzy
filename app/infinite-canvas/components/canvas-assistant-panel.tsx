...
import { AlignCenter, Copy, Grid3x3, History, ImagePlus, Layers, Maximize, MessageSquare, PanelRightClose, Plus, Scissors, Settings2, Shirt, Sparkles, Trash2, Type, Wand2, X } from "lucide-react";
import { Button, Modal, Segmented, Tooltip } from "@/components/ui/shadcn-compat";
import { motion } from "motion/react";

import { normalizeModelOptionValue, selectableModelsByCapability, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
import { canvasThemes } from "@/lib/canvas-theme";
import { nanoid } from "nanoid";
import { requestPlatformAgentResponse } from "@/services/api/canvas-agent";
...
const AGENT_SKILL_PROMPTS = [
    { id: "create-image", label: "新建图片生成节点", icon: ImagePlus, prompt: "在画布中央（坐标 0,0）创建一个 image 类型的生成配置节点，模式为图片，模型用 gpt-image-2，1:1 自动。" },
    { id: "create-text", label: "新建文本节点", icon: Type, prompt: "在画布中央（坐标 0,0）创建一个 text 类型的文本节点，标题「构思」，内容留空。" },
    { id: "center-align", label: "把节点居中对齐", icon: AlignCenter, prompt: "读取画布当前状态 canvas_get_state，把所有节点的 x 坐标都改成 0，让它们水平居中。如果有重叠就纵向错开 360 像素。" },
    { id: "grid-layout", label: "3×2 网格排版", icon: Grid3x3, prompt: "读取画布当前状态 canvas_get_state，把所有节点按类型排序后排成 3 列 × 2 行网格，列间距 360 行间距 300，原点放在 0,0。" },
    { id: "delete-empty", label: "删除空文本节点", icon: Trash2, prompt: "用 canvas_get_state 找到所有类型为 text 且内容为空的节点，调 canvas_delete_nodes 把它们删掉。" },
    { id: "zoom-fit", label: "缩放并居中", icon: Maximize, prompt: "用 canvas_set_viewport 把画布视图 scale 设为 1、x 和 y 都设为 0，缩放和居中一次到位。" },
] as const;
...
export const CanvasAssistantPanel = memo(function CanvasAssistantPanel({ nodes, selectedNodeIds, snapshot, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onApplyOps, canUndoOps, onUndoOps, onPasteImage, agentMode, onAgentModeChange, closing, onCollapse }: CanvasAssistantPanelProps) {
    ...
    const safeSessions = useMemo(() => (localSessions.length ? localSessions : [fallbackSession]), [fallbackSession, localSessions]);
    ...
    const historySessions = useMemo(() => safeSessions.filter((session) => session.messages.length > 0), [safeSessions]);
    const messages = activeSession?.messages || [];
    const hasMessages = messages.length > 0;
    const activeModel = "站内平台模型";
    ...
    const runOnlineAgentStep = async (sessionId: string, assistantId: string, history: CanvasAssistantMessage[], userMessage: CanvasAssistantMessage, loop: OnlineLoopContext) => {
        try {
            ...
            let streamed = "";
            const result = await requestPlatformAgentResponse(messages, ONLINE_AGENT_TOOLS, "required", (text) => {
                streamed = text;
    ...
    const continueOnlineToolLoopAfterResults = async (sessionId: string, assistantId: string, messages: ResponseInputMessage[], toolCalls: ResponseToolCall[], toolResults: OnlineExecutedToolCall[], step: number) => {
        ...
        let streamed = "";
        const next = await requestPlatformAgentResponse(nextMessages, ONLINE_AGENT_TOOLS, "auto", (text) => {
            streamed = text;
            if (text.trim()) upsertMessage(sessionId, { id: assistantId, role: "assistant", text });
        });
            ...
            {view === "setup" ? (
                ...
                <div className="thin-scrollbar min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
                    {view === "history" ? (
                        ...
                        <div className="flex h-full min-h-0 flex-col items-center justify-center gap-5 overflow-y-auto px-6 py-8">
                            <div className="space-y-1.5 text-center">
                                <div className="text-sm font-medium" style={{ color: theme.node.text }}>试试这些画布技能</div>
                                <div className="text-xs leading-5" style={{ color: theme.node.muted }}>点一下直接发给 Agent，或在输入框写自己的需求。</div>
                            </div>
                            <div className="flex flex-wrap justify-center gap-1.5">
                                {AGENT_SKILL_PROMPTS.map((skill) => (
                                    <button
                                        key={skill.id}
                                        type="button"
                                        className="inline-flex h-7 items-center gap-1.5 rounded-full border bg-transparent px-2.5 text-[12px] transition focus:outline-none focus-visible:ring-0"
                                        style={{
                                            borderColor: colorTheme === "dark" ? "rgba(255,255,255,.10)" : "rgba(0,0,0,.08)",
                                            color: theme.node.text,
                                        }}
                                        onMouseEnter={(event) => {
                                            (event.currentTarget as HTMLButtonElement).style.background = colorTheme === "dark" ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.04)";
                                        }}
                                        onMouseLeave={(event) => {
                                            (event.currentTarget as HTMLButtonElement).style.background = "transparent";
                                        }}
                                        onMouseDown={(event) => {
                                            (event.currentTarget as HTMLButtonElement).style.background = colorTheme === "dark" ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)";
                                        }}
                                        onMouseUp={(event) => {
                                            (event.currentTarget as HTMLButtonElement).style.background = colorTheme === "dark" ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.04)";
                                        }}
                                    ...
                                    >
                                        <skill.icon className="size-3.5 opacity-70" />
                                        {skill.label}
                                    </button>
                                ))}
                            </div>
    ...
    return (
        ...
        >
            ...
            >
                ...
                <header className="flex h-11 items-center justify-between border-b px-3" style={{ borderColor: theme.node.stroke }}>
                    <div className="flex min-w-0 items-center gap-2">
                        <span className="text-sm font-semibold leading-5">Agent</span>
                        <span className="text-[11px] opacity-50" style={{ color: theme.node.muted }}>画布助手</span>
                    </div>
                    <div className="flex shrink-0 items-center gap-1">
                        <Tooltip title={confirmTools ? "工具调用前会请求确认" : "工具自动执行"}>
                            <button
                                type="button"
                                onClick={() => setAgentState({ confirmTools: !confirmTools })}
                                className="inline-flex h-6 items-center gap-1.5 rounded-full px-2 text-[11px] transition hover:bg-stone-100 dark:hover:bg-white/10"
                                style={{ color: theme.node.muted }}
                            >
                                <span className="grid size-3.5 place-items-center rounded-full" style={{ background: confirmTools ? theme.node.activeStroke : "transparent", border: `1px solid ${confirmTools ? theme.node.activeStroke : theme.node.stroke}` }}>
                                    {confirmTools ? <span className="size-1.5 rounded-full bg-white" /> : null}
                                </span>
                                工具确认
                            </button>
                        </Tooltip>
                        <Tooltip title="收起对话">
                            <Button type="text" shape="circle" className="!h-7 !w-7 !min-w-7" style={iconButtonStyle} icon={<PanelRightClose className="size-4" />} onClick={collapse} />
                        </Tooltip>
                    </div>
                </header>
                {agentMode === "local" ? (
    ...
    return (
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                <div>
                    <div className="text-base font-semibold leading-6">平台 Agent 已启用</div>
                    <div className="mt-1 text-xs leading-5" style={{ color: theme.node.muted }}>
                        当前画布助手使用站内服务端模型配置，不需要连接本地 Agent，也不会要求用户填写 API Key。
                    </div>
                </div>
                <div className="rounded-lg border p-3" style={{ borderColor: theme.node.stroke }}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                            <div className="text-sm font-medium leading-5">运行模式</div>
                            <div className="mt-1 truncate text-xs leading-5" style={{ color: theme.node.muted }}>
                                {activeModel}
                            </div>
                        </div>
                        <span className="rounded-full border px-2.5 py-1 text-xs font-medium" style={{ borderColor: "rgba(22,163,74,.24)", color: "#16a34a", background: "rgba(22,163,74,.06)" }}>
                            已启用
                        </span>
                    </div>
                </div>
