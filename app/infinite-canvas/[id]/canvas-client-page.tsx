...
import { BookOpen, Bot, ChevronDown, Copy, Home, ImageIcon, Images, Layers, List, Maximize2, Menu, MessageSquare, Music2, Plus, Redo2, Settings2, Trash2, Undo2, Upload, Video, ZoomIn, ZoomOut } from "lucide-react";
import { saveAs } from "file-saver";

import { createPlatformImageGenerationTask, requestEdit, requestImageQuestion, waitForPlatformImageGenerationTask, type PlatformImageGenerationTask } from "@/services/api/image";
import { requestAudioGeneration, storeGeneratedAudio } from "@/services/api/audio";
import { requestVideoGeneration, storeGeneratedVideo } from "@/services/api/video";
import { DOCS_URL } from "@/constant/env";
import { defaultConfig, type AiConfig, useEffectiveConfig } from "@/stores/use-config-store";
...
import { INFINITE_CANVAS_LOCAL_AGENT_ENABLED, INFINITE_CANVAS_PLATFORM_AGENT_ENABLED } from "@/lib/infinite-canvas-public-env";
import { UserStatusActions } from "@/components/layout/user-status-actions";
import { useAssetStore } from "@/stores/use-asset-store";
import { useThemeStore } from "@/stores/use-theme-store";
import { cropDataUrl, splitDataUrl, upscaleDataUrl } from "../utils/canvas-image-data";
import { fitNodeSize, nodeSizeFromRatio } from "../utils/canvas-node-size";
import { App, Button, Dropdown, Modal } from "@/components/ui/shadcn-compat";
import { NODE_DEFAULT_SIZE, NODE_MIN_SIZE, getNodeSpec } from "../constants";
...
const EMPTY_RESOURCE_REFERENCES: ReturnType<typeof buildNodeMentionReferences> = [];
...
function normalizeCanvasAgentMode(mode: CanvasAgentMode): CanvasAgentMode {
    if (mode === "local" && !INFINITE_CANVAS_LOCAL_AGENT_ENABLED && INFINITE_CANVAS_PLATFORM_AGENT_ENABLED) return "online";
    if (mode === "online" && !INFINITE_CANVAS_PLATFORM_AGENT_ENABLED && INFINITE_CANVAS_LOCAL_AGENT_ENABLED) return "local";
    return mode;
}
...
function InfiniteCanvasPage() {
    ...
    const [agentMode, setAgentMode] = useState<CanvasAgentMode>(() => normalizeCanvasAgentMode("online"));
    ...
    const resumedImageTaskKeysRef = useRef(new Set<string>());
    ...
    const resumeImageGenerationNode = useCallback(
        async (node: CanvasNodeData) => {
            const taskId = node.metadata?.generationTaskId;
            if (!taskId) return;
            const controller = startGenerationRequest(node.id, node.id, node.id);
            try {
                const task: PlatformImageGenerationTask = {
                    provider: "platform",
                    id: taskId,
                    path: node.metadata?.generationTaskPath || "/api/general-image",
                    model: node.metadata?.model || effectiveConfig.imageModel || effectiveConfig.model,
                    mode: node.metadata?.generationType === "edit" ? "image-to-image" : "text-to-image",
                };
                const urls = await waitForPlatformImageGenerationTask(task, { signal: controller.signal });
                const uploaded = await uploadImage(urls[0]);
                const imageConfig = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
                const imageSize = fitNodeSize(uploaded.width, uploaded.height, imageConfig.width, imageConfig.height);
                setNodes((prev) => {
                    const current = prev.find((item) => item.id === node.id);
                    const rootId = current?.metadata?.batchRootId;
                    return prev.map((item) => {
                        ...
                        if (item.id === node.id) {
                            return {
                                ...nextBase,
                                metadata: clearImageGenerationTaskMetadata({ ...item.metadata, ...imageMetadata(uploaded), errorDetails: undefined }),
                            };
                        }
                        if (!item.metadata?.primaryImageId || item.metadata.primaryImageId === node.id) {
                            return {
                                ...nextBase,
                                metadata: clearImageGenerationTaskMetadata({ ...item.metadata, ...imageMetadata(uploaded), primaryImageId: node.id, errorDetails: undefined }),
                            };
                        }
                        return item;
                    });
                ...
                const errorDetails = error instanceof Error ? error.message : "Image generation failed";
                setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails } } : item)));
            } finally {
                finishGenerationRequest(node.id, controller);
            }
        },
        [effectiveConfig.imageModel, effectiveConfig.model, finishGenerationRequest, startGenerationRequest],
    );
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            const restoredNodes = await hydrateCanvasImages(restoreInterruptedGeneration(project.nodes));
    ...
    useEffect(() => {
        resumedImageTaskKeysRef.current.clear();
    }, [projectId]);

    useEffect(() => {
        if (!projectLoaded) return;
        const pendingImageNodes = nodesRef.current.filter((node) => node.type === CanvasNodeType.Image && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.generationTaskId);
        pendingImageNodes.forEach((node) => {
            const key = `${node.id}:${node.metadata?.generationTaskId}`;
            if (resumedImageTaskKeysRef.current.has(key)) return;
            resumedImageTaskKeysRef.current.add(key);
            void resumeImageGenerationNode(node);
        });
    }, [projectLoaded, resumeImageGenerationNode]);
                ...
                if (mode === "image") {
                    ...
                    await Promise.all(
                        targetIds.map(async (targetId) => {
                            try {
                                const task = await createPlatformImageGenerationTask({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, { signal: controller.signal });
                                setNodes((prev) => prev.map((node) => (node.id === targetId ? { ...node, metadata: { ...node.metadata, ...imageGenerationTaskMetadata(task), errorDetails: undefined } } : node)));
                                const urls = await waitForPlatformImageGenerationTask(task, { signal: controller.signal });
                                const uploaded = await uploadImage(urls[0]);
                                const imageSize = fitNodeSize(uploaded.width, uploaded.height, imageConfig.width, imageConfig.height);
                                setNodes((prev) => {
                                    ...
                                    return prev.map((node) => {
                                        ...
                                        if (node.id === rootId && (targetId === rootId || !root?.metadata?.primaryImageId))
                                            return {
                                                ...
                                                width: imageSize.width,
                                                height: imageSize.height,
                                                metadata: clearImageGenerationTaskMetadata({ ...node.metadata, ...imageMetadata(uploaded), primaryImageId: targetId, errorDetails: undefined }),
                                            };
                                        if (node.id === targetId)
                                            return {
                                                ...node,
                                                ...
                                                width: imageSize.width,
                                                height: imageSize.height,
                                                metadata: clearImageGenerationTaskMetadata({ ...node.metadata, ...imageMetadata(uploaded), errorDetails: undefined }),
                                            };
                                        return node;
                                    });
                                });
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                const imageTask = await createPlatformImageGenerationTask(generationConfig, prompt, useReferenceImages ? retryImages : [], { signal: controller.signal });
                setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, ...imageGenerationTaskMetadata(imageTask), errorDetails: undefined } } : item)));
                const imageUrls = await waitForPlatformImageGenerationTask(imageTask, { signal: controller.signal });
                const uploadedImage = await uploadImage(imageUrls[0]);
                const imageConfig = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
                    ...
                    prev.map((item) =>
                        item.id === node.id
                            ? {
                                  ...
                                  width: imageSize.width,
                                  height: imageSize.height,
                                  metadata: clearImageGenerationTaskMetadata({ ...item.metadata, ...imageMetadata(uploadedImage), prompt, ...generationMetadata, errorDetails: undefined }),
                              }
                            : item,
                    ),
                );
    ...
    const effectiveAgentMode = normalizeCanvasAgentMode(agentMode);
    const setSafeAgentMode = useCallback((mode: CanvasAgentMode) => setAgentMode(normalizeCanvasAgentMode(mode)), []);
    const openAgent = useCallback((mode: CanvasAgentMode = effectiveAgentMode) => {
        if (agentCloseTimerRef.current) {
            clearTimeout(agentCloseTimerRef.current);
            agentCloseTimerRef.current = null;
        }
        setSafeAgentMode(mode);
        setAssistantMounted(true);
        setAssistantClosing(false);
        setAssistantCollapsed(false);
    }, [effectiveAgentMode, setSafeAgentMode]);
        ...
        <main className="flex h-screen min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onDuplicateSelection={() => {
                        ...
                        if (firstSelected) duplicateNode(firstSelected);
                    }}
                    onZoomToFit={resetViewport}
                    onZoomIn={() => setZoomScale(Math.min(viewport.k + 0.1, 5))}
                    onZoomOut={() => setZoomScale(Math.max(viewport.k - 0.1, 0.05))}
                ...
                <InfiniteCanvas
                    ...
                    onDrop={handleDrop}
                >
                    <svg className="absolute left-0 top-0 h-[10000px] w-[10000px] overflow-visible" style={{ pointerEvents: "none", transform: "translateZ(0)", zIndex: 0, maxWidth: "none" }}>
                        {connections
                            .filter((connection) => {
            ...
            {assistantMounted ? (
                <CanvasAssistantPanel
                    ...
                    onAgentModeChange={setSafeAgentMode}
...
function CanvasTopBar({
    ...
    const menuItemClass = "inline-flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm transition hover:bg-stone-100 dark:hover:bg-stone-800";

    return (
        <>
            <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-14 items-center justify-between px-3">
                <div className="pointer-events-auto flex min-w-0 items-center gap-1">
                    <Dropdown
                        ...
                        dropdownRender={(menu) => (
                            <div className="min-w-[240px] rounded-lg border border-stone-200 bg-white p-1 shadow-[0_8px_24px_rgba(0,0,0,.08)] dark:border-stone-700 dark:bg-stone-900">
                                {menu}
                            </div>
                        )}
                        menu={{
                            items: [
                                { key: "home", label: <button type="button" className={menuItemClass} onClick={onHome}><Home className="size-4 opacity-70" />主页</button> },
                                { type: "divider" },
                                { key: "new", label: <button type="button" className={menuItemClass} onClick={onCreateProject}><Plus className="size-4 opacity-70" />新建画布</button> },
                                { key: "projects", label: <button type="button" className={menuItemClass} onClick={onProjects}><Images className="size-4 opacity-70" />我的画布</button> },
                                { key: "import", label: <button type="button" className={menuItemClass} onClick={onImportImage}><Upload className="size-4 opacity-70" />导入素材</button> },
                                { key: "delete", label: <button type="button" className={cn(menuItemClass, "text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30")} onClick={onDeleteProject}><Trash2 className="size-4" />删除当前画布</button> },
                                { type: "divider" },
                                { key: "undo", disabled: !canUndo, label: <button type="button" className={menuItemClass} onClick={onUndo} disabled={!canUndo}><Undo2 className="size-4 opacity-70" />撤销<span className="ml-auto text-xs opacity-40">⌘ Z</span></button> },
                                { key: "redo", disabled: !canRedo, label: <button type="button" className={menuItemClass} onClick={onRedo} disabled={!canRedo}><Redo2 className="size-4 opacity-70" />重做<span className="ml-auto text-xs opacity-40">⌘ ⇧ Z</span></button> },
                                { key: "duplicate", label: <button type="button" className={menuItemClass} onClick={onDuplicateSelection}><Copy className="size-4 opacity-70" />复制选中<span className="ml-auto text-xs opacity-40">⌘ D</span></button> },
                                { type: "divider" },
                                { key: "fit", label: <button type="button" className={menuItemClass} onClick={onZoomToFit}><Maximize2 className="size-4 opacity-70" />缩放到合适<span className="ml-auto text-xs opacity-40">⇧ 1</span></button> },
                                { key: "zin", label: <button type="button" className={menuItemClass} onClick={onZoomIn}><ZoomIn className="size-4 opacity-70" />放大<span className="ml-auto text-xs opacity-40">⌘ +</span></button> },
                                { key: "zout", label: <button type="button" className={menuItemClass} onClick={onZoomOut}><ZoomOut className="size-4 opacity-70" />缩小<span className="ml-auto text-xs opacity-40">⌘ -</span></button> },
                            ],
                        }}
                    >
                        <button
                            type="button"
                            className="grid size-8 place-items-center rounded-md text-stone-600 transition hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-800"
                            aria-label="打开画布菜单"
                            onPointerDown={(event) => event.stopPropagation()}
                            onMouseDown={(event) => event.stopPropagation()}
                        >
                    ...
                    <button
                        type="button"
                        className="ml-1 inline-flex h-8 items-center gap-1.5 rounded-md px-2 text-sm font-medium text-stone-700 transition hover:bg-stone-100 dark:text-stone-200 dark:hover:bg-stone-800"
                        onClick={() => onProjects()}
                        title="返回画布列表"
                    >
                ...
                <div className="pointer-events-auto flex items-center gap-2">
                    ...
                    <button
                        type="button"
                        className="inline-flex h-8 items-center gap-1.5 rounded-md border border-stone-200 bg-white px-2.5 text-sm font-medium text-stone-700 transition hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-200 dark:hover:bg-stone-800"
                        style={agentOpen ? { background: theme.toolbar.activeBg, color: theme.node.text, borderColor: theme.node.activeStroke } : undefined}
...
function imageGenerationTaskMetadata(task: PlatformImageGenerationTask): CanvasNodeMetadata {
    return {
        generationTaskProvider: task.provider,
        generationTaskId: task.id,
        generationTaskPath: task.path,
        generationTaskStartedAt: new Date().toISOString(),
    };
}

function clearImageGenerationTaskMetadata(metadata: CanvasNodeMetadata): CanvasNodeMetadata {
    const next = { ...metadata };
    delete next.generationTaskProvider;
    delete next.generationTaskId;
    delete next.generationTaskPath;
    delete next.generationTaskStartedAt;
    return { ...next, status: NODE_STATUS_SUCCESS, errorDetails: undefined };
}
...
function restoreInterruptedGeneration(nodes: CanvasNodeData[]) {
    return normalizeNodeSizes(nodes).map((node) => {
        if (node.metadata?.status !== "loading") return node;
        if (canResumeImageGeneration(node, nodes)) return { ...node, metadata: { ...node.metadata, errorDetails: undefined } };
        return { ...node, metadata: { ...node.metadata, status: "error" as const, errorDetails: "页面刷新后生成已中断，请重新生成。" } };
    });
}

function normalizeNodeSizes(nodes: CanvasNodeData[]) {
    // Auto-resize any persisted node that is smaller than its type's minimum size
    // (e.g. legacy 280x200 config nodes whose content would otherwise be clipped).
    return nodes.map((node) => {
        const min = NODE_MIN_SIZE[node.type];
        if (!min) return node;
        const width = Math.max(node.width, min.minWidth);
        const height = Math.max(node.height, min.minHeight);
        if (width === node.width && height === node.height) return node;
        return { ...node, width, height };
    });
}

function canResumeImageGeneration(node: CanvasNodeData, nodes: CanvasNodeData[]) {
    if (node.type !== CanvasNodeType.Image) return false;
    if (node.metadata?.generationTaskId) return true;
    if (!node.metadata?.isBatchRoot) return false;
    const childIds = node.metadata.batchChildIds || [];
    return childIds.some((id) => nodes.some((item) => item.id === id && item.metadata?.generationTaskId));
}
