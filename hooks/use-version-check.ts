...
import {
    INFINITE_CANVAS_CHANGELOG_URL,
    INFINITE_CANVAS_VERSION_CHECK_ENABLED,
    INFINITE_CANVAS_VERSION_URL,
} from "@/lib/infinite-canvas-public-env";
import { parseChangelog, type ReleaseInfo } from "@/lib/release";

const latestVersionUrl = INFINITE_CANVAS_VERSION_URL;
const latestChangelogUrl = INFINITE_CANVAS_CHANGELOG_URL;
...
export function useVersionCheck() {
    ...
    const checkLatestVersion = useCallback(async () => {
        if (!INFINITE_CANVAS_VERSION_CHECK_ENABLED) return false;
        try {
